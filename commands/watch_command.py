...
from monitoring.queries import Queries
    ...
    def execute(self, args):
        ...
        while i < len(args):
            ...
            if arg in ("-i", "--interval"):
                ...
                try:
                    ...
                    return
            elif arg.startswith("-") and not arg in ("-m", "--metrics"): # Ignoring old metrics flag
                print(f"Unknown option: {arg}")
                self._print_usage()
                return
            elif arg in ("-m", "--metrics"):
                i += 1 # Just skip the old metrics argument
            else:
        ...
        try:
            while count is None or loop_count < count:
                # Use os.system instead of os.subprocess which does not exist
                ...
                try:
                    ...
                    metrics_data["requests_per_sec"] = self.prom_client.query(queries["requests_per_sec"])
                    metrics_data["error_rate"] = self.prom_client.query(queries["error_rate"])
                    metrics_data["latency_p95"] = self.prom_client.query(queries["latency_p95"])
                ...
                if metrics_data.get("cpu") is not None:
                    # CPU is in seconds/second, multiply by 100 for %
                ...
                if metrics_data.get("latency_p95") is not None and str(metrics_data["latency_p95"]).lower() != "nan":
                    lat_val = float(metrics_data["latency_p95"]) * 1000 # convert to ms
